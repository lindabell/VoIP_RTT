...
void tftp_get(const char* host, const char* dir, const char* filename)
{
	...
	if (fd < 0)
	{
		rt_kprintf("can't open local filename %s\n", tftp_buffer);
		return;
	}
...
FINSH_FUNCTION_EXPORT(tftp_get,eg:tftp_get("HostIp","LocalPath","FileName"));
...
FINSH_FUNCTION_EXPORT(tftp_put,eg:tftp_put("HostIp","LocalPath","FileName"));
