...
//	write_reg(0xC7,0x01);
    ...
    /* clear */
    {
        ...
        for(i=0; i<800*480; i++)
        {
            LCD_DataWrite(0xFFFF);  //write red data
        }
    }
...
static void lcd_gradual(void)
{
    ...
    /* B */
    {
        for(i=lcd_info.height/3*0;
        i<(lcd_info.height/3*0) + (lcd_info.height/3);
        i++)
        {
            for(j=0; j<0x1F; j++)
            {
                pixel = j;
                graphic_ops->set_pixel((const char*)&pixel,
                                       (j*3+0) + x,
                                       i);
                graphic_ops->set_pixel((const char*)&pixel,
                                       (j*3+1) + x,
    ...
    /* G */
    {
        for(i=lcd_info.height/3*1;
        i<(lcd_info.height/3*1) + (lcd_info.height/3);
        i++)
        {
            for(j=0; j<0x3F; j++)
            {
    ...
    /* R */
    {
        for(i=lcd_info.height/3*2;
        i<(lcd_info.height/3*2) + (lcd_info.height/3);
        i++)
        {
            for(j=0; j<0x1F; j++)
            {
    ...
    {
        pixel = 0x0000;
        for(i=x; i<lcd_info.width; i++)
        {
            graphic_ops->draw_vline((const char *)&pixel,
                                    i,
                                    0,
                                    lcd_info.height);
        }
...
void lcd_test(void)
{
    lcd_control(&_lcd_device, RTGRAPHIC_CTRL_GET_INFO, &lcd_info);
    graphic_ops = &ra8875_ops;
