...
#define AUDIO_I2S_RX_DMA_CHANNEL    DMA_Channel_3
...
static void NVIC_Configuration(void)
{
    ...
    NVIC_InitStructure.NVIC_IRQChannel = AUDIO_I2S_TX_DMA_IRQ;
...
static void DMA_TX_Configuration(rt_uint32_t addr, rt_size_t size)
{
    DMA_InitTypeDef DMA_InitStructure;

    /* Configure the DMA Stream */
    DMA_Cmd(AUDIO_I2S_TX_DMA_STREAM, DISABLE);
    DMA_DeInit(AUDIO_I2S_TX_DMA_STREAM);

    /* Set the parameters to be configured */
    DMA_InitStructure.DMA_Channel = AUDIO_I2S_TX_DMA_CHANNEL;
    DMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)(&AUDIO_I2S_TX_PORT->DR);
    ...
    DMA_Init(AUDIO_I2S_TX_DMA_STREAM, &DMA_InitStructure);

    /* Enable SPI DMA Tx request */
    SPI_I2S_DMACmd(AUDIO_I2S_TX_PORT, SPI_I2S_DMAReq_Tx, ENABLE);

    DMA_ITConfig(AUDIO_I2S_TX_DMA_STREAM, DMA_IT_TC, ENABLE);
    DMA_Cmd(AUDIO_I2S_TX_DMA_STREAM, ENABLE);
}

static void DMA_RX_Configuration(rt_uint32_t addr, rt_size_t size)
{
    DMA_InitTypeDef DMA_InitStructure;

    /* Configure the DMA Stream */
    DMA_Cmd(AUDIO_I2S_RX_DMA_STREAM, DISABLE);
    DMA_DeInit(AUDIO_I2S_RX_DMA_STREAM);

    /* Set the parameters to be configured */
    DMA_InitStructure.DMA_Channel = AUDIO_I2S_RX_DMA_CHANNEL;
    DMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)(&AUDIO_I2S_RX_PORT->DR);
    DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)addr;      /* This field will be configured in play function */
    ...
    DMA_InitStructure.DMA_MemoryBurst = DMA_MemoryBurst_Single;
    DMA_InitStructure.DMA_PeripheralBurst = DMA_PeripheralBurst_Single;
    DMA_Init(AUDIO_I2S_RX_DMA_STREAM, &DMA_InitStructure);

    /* Enable SPI DMA Tx request */
    SPI_I2S_DMACmd(AUDIO_I2S_RX_PORT, SPI_I2S_DMAReq_Rx, ENABLE);

    DMA_ITConfig(AUDIO_I2S_RX_DMA_STREAM, DMA_IT_TC, ENABLE);
    DMA_Cmd(AUDIO_I2S_RX_DMA_STREAM, ENABLE);
}
...
#endif
    I2S_Init(AUDIO_I2S_TX_PORT, &I2S_InitStructure);
    I2S_FullDuplexConfig(AUDIO_I2S_RX_PORT, &I2S_InitStructure);
}
...
static rt_err_t codec_close(rt_device_t dev)
{
    ...
    if (r06 & MS)
    {
        NVIC_DisableIRQ(AUDIO_I2S_TX_DMA_IRQ);
        DMA_Cmd(AUDIO_I2S_TX_DMA_STREAM, DISABLE);
        /* Clear DMA Stream Transfer Complete interrupt pending bit */
        DMA_ClearITPendingBit(AUDIO_I2S_TX_DMA_STREAM, AUDIO_I2S_TX_DMA_IT_TC);

        while ((AUDIO_I2S_TX_PORT->SR & SPI_I2S_FLAG_TXE) == 0);
        while ((AUDIO_I2S_TX_PORT->SR & SPI_I2S_FLAG_BSY) != 0);
        I2S_Cmd(AUDIO_I2S_TX_PORT, DISABLE);
...
static void codec_dma_isr(void)
{
    ...
    if (next_index != codec.put_index)
    {
        ...
        if ((r06 & MS) == 0)
        {
//            AUDIO_I2S_TX_PORT->I2SCFGR |= SPI_I2SCFGR_I2SE;
            r06 |= MS;
//            codec_send(r06);
        }
#endif
...
{
    /* Test on DMA Stream Transfer Complete interrupt */
    if(DMA_GetITStatus(AUDIO_I2S_TX_DMA_STREAM, AUDIO_I2S_TX_DMA_IT_TC))
    {
        /* Clear DMA Stream Transfer Complete interrupt pending bit */
        DMA_ClearITPendingBit(AUDIO_I2S_TX_DMA_STREAM, AUDIO_I2S_TX_DMA_IT_TC);
